...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		// (x-center)^2 + (y-center)^2 + (z-center)^2 - radius^2
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		// double tmp = sqrd(_x[0]-m_center[0])
		double tmp = sqrd(_x[0]-m_center[0])  + sqrd(_x[1]-m_center[1]) + sqrd(_x[2]-m_center[2]) + sqrd(m_radius) - sqrd(m_a);
		return sqrd(tmp) - 4.0 * sqrd(m_radius) * (sqrd(_x[0]-m_center[0])  + sqrd(_x[1]-m_center[1]));
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for(unsigned int i = 0; i < m_numCenters; i++) {
			// first term of f(x): multiply alpha coefficients with the norm of p_i - x
			result += m_coefficents[i] * EvalBasis((m_funcSamp.m_pos[i] - _x).norm());
		}

		// Second term of f(x) : multiply b coefficients with x
		result += m_coefficents[m_numCenters+0] * _x[0];
		result += m_coefficents[m_numCenters+1] * _x[1];
		result += m_coefficents[m_numCenters+2] * _x[2];

		// Third term of f(x) : the constant d
