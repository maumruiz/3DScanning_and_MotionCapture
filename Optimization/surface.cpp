...
struct SurfaceCostFunction
{
	...
	{
		const auto firstTerm = T(point.x) * T(point.x) / T(*a);
		const auto secondTerm = T(point.y) * T(point.y) / T(*b);
		const auto res = (firstTerm - secondTerm) / T(*c);
		residual[0] = T(point.z) - res;
		return true;
	}
